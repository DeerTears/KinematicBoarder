...
export var velocity_length: float = 0
...
func _physics_process(delta):
	...
	if is_on_floor():
		...
		velocity.y = -2
		if is_on_ramp:
			launched_velocity = velocity * 1.65
			#launched_velocity = launched_velocity.rotated(Vector3.FORWARD, deg2rad(-45)) # works in one direction, doesn't remove itself after launching
		else:
			launched_velocity = velocity * 1.25
			was_on_floor = false
	else:
		rotation = rotation.move_toward(Vector3.UP, delta * 0.2)
		turn_amount *= 0.6
	...
	if is_on_floor(): # this check isn't reliable, I don't have a snapping solution for sharp ramps and a ball collider :(
		...
		if is_on_ramp:
			rotated_magnitude *= 1.65
		if jump_strength == 0:
			jump_was_pressed = true
		if jump_strength == -1 and jump_was_pressed:
			jump_was_pressed = false
			velocity += current_floor_normal * 40
...
func _on_RampDetector_body_entered(body):
	if "StaticBody" in body.name:
		is_on_ramp = true

func _on_RampDetector_body_exited(body):
	if "StaticBody" in body.name:
		is_on_ramp = false
